...
async function generateWorkflowCode() {
  ...
  const { data } = await supabase
    ...
    .select('code')
    .ilike('code', `${prefix}%`)
    .order('code', { ascending: false }); // Lấy toàn bộ thay vì limit(1) để có thể lọc thủ công các code rác

  if (data && data.length > 0) {
    // Tìm mã lớn nhất thực sự là số hợp lệ
    let maxNum = 0;
...
async function autoCreateWorkflowsForEntity(tableName, entityRecord, currentUser) {
  ...
  if (tableName === 'crm_orders') {
    ...
    const hasPhotocopy = (items || []).some(item =>
      item.product && item.product.category && String(item.product.category).toLowerCase().includes('máy photocopy')
    );
