...
**Nguồn `card_type`:** form Tạo đơn có **radio "Loại đơn": `Máy bán` / `Máy thuê` / `Vật tư`** — **mọi phòng** chọn được cả 3 (KD và KT đều bán được cả 3 loại). Lưu vào `crm_orders` (vd cột `order_type`); hook `createCardsFromOrder` đọc thẳng → KHÔNG đoán từ sản phẩm/`is_for_rent`. `owner_dept` của thẻ = **phòng tạo đơn** (không hardcode KD=máy/KT=vật tư).

**Ai THẤY (visibility) — theo `owner_dept` (phòng tạo đơn), không theo phòng cố định:**
- *Luồng thương mại:* **phòng tạo đơn** (người bán) thấy cả 4 cột + **thấy giá**. KTHC thấy từ "Lên hóa đơn" trở đi + thấy giá. Phòng khác KHÔNG thấy.
- *Luồng kỹ thuật* (chỉ máy bán/thuê): **KT thấy & kéo** (luôn là việc của KT, dù ai bán); **phòng tạo đơn thấy read-only** để theo dõi lắp; KTHC không thấy. **Thẻ kỹ thuật KHÔNG có field tài chính** (vô giá, dù ai lắp).
- **Giá đi theo người bán:** phòng nào tạo đơn thì thấy giá đơn đó. KT thấy giá máy **KT tự bán**, KHÔNG thấy giá máy **KD bán** (đơn KD, KT không mở được), KHÔNG thấy giá trên thẻ lắp máy.
...
1. Lấy `card_type` từ **radio "Loại đơn"** trên form (`Máy bán`→`ban_may`, `Máy thuê`→`thue_may`, `Vật tư`→`ban_vat_tu`) — lưu ở `crm_orders.order_type`. KHÔNG đoán từ sản phẩm. `owner_dept` = phòng tạo đơn (KD hoặc KT đều được, cho cả 3 loại).
...
**Mỗi phòng chỉ thấy/mở đơn của phòng mình** ở tab "Bán hàng" (server lọc đơn theo `owner_dept` — guard `isKTRestricted` đã làm, mở rộng cho mọi phòng). Nhờ vậy: thợ lắp KT **không mở được đơn máy của KD** → không thấy giá KD bán; còn giá đơn **KT tự bán** thì KT thấy (đúng vai người bán). Thẻ kỹ thuật vô giá nên không lộ gì khi lắp.
